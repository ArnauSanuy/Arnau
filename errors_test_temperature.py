...
values_one_minus_alphac = []

for _m_dot in m_dot:
    _x_m = x_m_appendix_B(_m_dot)
    minus_alpha_c = one_minus_alpha_c(theta_e, _x_m, m, _m_dot)
    values_one_minus_alphac.append(minus_alpha_c)

one_values = one_minus_alpha_c(theta_e, x_m, m, m_dot)
print(one_values)
...
for _m_dot in m_dot:
    ...
    P_compton_values = P_compton(P_synch_values, _alpha_c, theta_e)
    P_bremms_values = P_bremms(theta_e)
    P_total_values = P_total(theta_e, _x_m, m, _m_dot)
    equilibrium = np.abs(Q_e_plus_values - P_synch_values - P_compton_values)
...
ax.plot(np.log10(m_dot), np.asarray(T_eq_values) / 1e9, label="implementation")
...
# ax.semilogy(np.log10(m_dot), np.asarray(x_m_values), label="implementation")
# ax.set_xlabel(r"$Log(\dot{m})$")
# ax.set_ylabel("x_m")
# ax.set_xlim([0, -8])
# ax.set_ylim([8e1, 2e3])
