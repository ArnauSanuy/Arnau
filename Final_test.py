...
m_proof = 5e7
_new_m_dot = new_m_dot(P6, T7)
new_x_m = x_m_appendix_B(_new_m_dot)
new_alpha_c = alpha_c(theta_from_T(T_final_values), _new_m_dot)
mu_p = vp(new_x_m, _new_m_dot, m_proof, theta_from_T(T_eq_values * u.K)).value
mean = mean(mu_p)
...
first_flux = Total_flux(m_proof, _new_m_dot, new_x_m, T_eq_values, new_alpha_c, v, v1, v2, mean)
calculus = np.log10(first_flux)
...
plt.plot(np.log10(v1), np.log10(first_flux[0]), color = 'b') 
plt.plot(np.log10(v2), np.log10(first_flux[1]), color = 'b')
plt.show()
